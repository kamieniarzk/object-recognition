import numpy as np
import math
import geometric_constants as constants


def is_lot_logo(image: np.ndarray, path=None):
    coefficients = Coefficients(image)
    if path is not None:
        coefficients.write_to_file(path + '-coefficients.txt')
    if coefficients.area == 0:
        return False
    if constants.M3_MIN <= coefficients.M3 <= constants.M3_MAX and \
            constants.M4_MIN <= coefficients.M4 <= constants.M4_MAX and \
            constants.M5_MIN <= coefficients.M5 <= constants.M5_MAX and \
            constants.M7_MIN <= coefficients.M7 <= constants.M7_MAX and \
            constants.M9_MIN <= coefficients.M9 <= constants.M9_MAX and \
            constants.W3_MIN <= coefficients.W3 <= constants.W3_MAX and \
            constants.W4_MIN <= coefficients.W4 <= constants.W4_MAX:
        return True
    return False
...
class Coefficients:
    ...
    def write_to_file(self, file_name: str):
        ...
        f.write('Area: {0:.10f}\n'.format(self.area))
        f.write('Perimeter/area ratio: {0:.10f}\n'.format(self.perimeter / self.area))
